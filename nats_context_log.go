...
* @return: void
 */
func (ctx *natsContext) LogInfo(format string, args ...any) {
...
* @return: void
 */
func (ctx *natsContext) LogDebug(format string, args ...any) {
	logStr, logInfo := ctx.format(format, args...)
	logInfo.Level = "DEBUG"
	logStr = "[DEBUG] " + logStr
	log.Println(logStr)
...
* @return: void
 */
func (ctx *natsContext) LogError(format string, args ...any) {
	logStr, logInfo := ctx.format(format, args...)
	logInfo.Level = "ERROR"
	logStr = "[ERROR] " + logStr
	log.Println(logStr)
...
* @return: void
 */
func (ctx *natsContext) LogWarning(format string, args ...any) {
	logStr, logInfo := ctx.format(format, args...)
...
* @return: void
 */
func (ctx *natsContext) LogFatal(format string, args ...any) {
	logStr, logInfo := ctx.format(format, args...)
	logInfo.Level = "FATAL"
	logStr = "[FATAL] " + logStr
	log.Fatalln(logStr)
...
func (ctx *natsContext) LogPanic(format string, args ...any) {
	logStr, logInfo := ctx.format(format, args...)
	logInfo.Level = "PANIC"
	logStr = "[PANIC] " + logStr
	log.Panicln(logStr)
...
* @return: string
 */
func (ctx *natsContext) format(format string, args ...any) (string, logMessage) {
	// Format the ctx
	logStr := fmt.Sprintf(format, args...)
	logInfo := logMessage{
		RequestID: ctx.request.ContextId,
		Message:   logStr,
	}
	...
	if ok {
		...
		logStr = fmt.Sprintf("%s:%d:%s RequestID: %s, Message: %s", file, line, functionName, ctx.request.ContextId, logStr)
	}
